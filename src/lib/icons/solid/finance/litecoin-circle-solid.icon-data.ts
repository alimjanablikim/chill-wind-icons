import type { IconData } from '../../icon-data.interface';

/**
 * @category finance
 * @type solid
 * @name Litecoin Circle
 */
export const litecoinCircleSolidIconData: IconData = {
  name: 'litecoin-circle',
  type: 'solid',
  category: 'finance',
  viewBox: '0 0 24 24',
  innerSvg: `<path fill="currentColor" fill-rule="evenodd" d="M1.25 12C1.25 6.063 6.063 1.25 12 1.25S22.75 6.063 22.75 12 17.937 22.75 12 22.75 1.25 17.937 1.25 12m9.25-5.75a.75.75 0 0 1 .75.75v3.957l1.445-.642a.75.75 0 0 1 .61 1.37l-2.055.913v3.652h4.25a.75.75 0 0 1 0 1.5h-4.4a1.35 1.35 0 0 1-1.35-1.35v-3.135l-.945.42a.75.75 0 0 1-.61-1.37l1.555-.691V7a.75.75 0 0 1 .75-.75" clip-rule="evenodd"/>`,
  defaultStrokeWidth: null,
};
